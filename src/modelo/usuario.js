...
export class Usuario {
    ...
    listar = async (id) => {
        const sql =
            ...
            where u.eliminado = false and rol.numero > 1 and u.id != ${pool.escape(id.usuario)} and u.idempresa = ${pool.escape(id.empresa)}
    ...
    listarReciclaje = async (datos) => {
        const sql =
            ...
            u.apellido2,u.celular, u.direccion
            from usuario u inner join rol on rol.id = u.idrol
            where eliminado = true and rol.numero > 1 and u.idempresa = ${pool.escape(datos.empresa)}
    ...
    buscar = async (dato) => {
        ...
        const sql =
            ...
            u.apellido2  like '${dato.dato}%') and u.eliminado = false and rol.numero > 1  and u.id != ${pool.escape(dato.usuario)} and u.idempresa = ${pool.escape(dato.empresa)}
            ORDER by id`;
        const [rows] = await pool.query(sql)
        return rows
    }
    ...
    buscarEliminado = async (dato) => {
        ...
        const sql =
            ...
            u.ci like '${dato.dato}%' or
            u.apellido1  like '${dato.dato}%' or
            u.apellido2  like '${dato.dato}%') and u.eliminado = true and rol.numero > 1 and u.idempresa = ${pool.escape(dato.empresa)}
            ORDER by id`;
        const [rows] = await pool.query(sql)
        return rows
    }
    ...
    listarSiguiente = async (id) => {
        const sql =
            ...
            u.apellido2,u.celular, u.direccion
            from usuario u inner join rol on rol.id = u.idrol
            where eliminado = false and rol.numero > 1 and u.id != ${pool.escape(id.usuario)} and u.idempresa = ${pool.escape(id.empresa)}
    ...
    listarSiguienteEliminados = async (id) => {
        const sql =
            ...
            u.apellido2,u.celular, u.direccion
            from usuario u inner join rol on rol.id = u.idrol
            where eliminado = true and rol.numero > 1 and u.idempresa = ${pool.escape(id.empresa)}
            and u.id < ${pool.escape(id.id)} ORDER by u.id DESC  limit 10`;
        const [rows] = await pool.query(sql)
        return rows
    }
    ...
    listarAnteriorEliminados = async (id) => {
        const sql =
            ...
            u.apellido2,u.celular, u.direccion
            from usuario u inner join rol on rol.id = u.idrol
            where eliminado = false and rol.numero > 1 and u.idempresa = ${pool.escape(id.empresa)}
            and u.id > ${pool.escape(id.id)} and eliminado = true limit 10`;
        const [rows] = await pool.query(sql)
        rows.reverse()
        return rows
    }
    ...
    listarRol = async () => {
        const sql =
            `SELECT id as id, rol as nombre from rol where numero > 1`;
        const [rows] = await pool.query(sql)
    ...
    restaurar = async (datos) => {
        const sql = `update usuario set eliminado = false, modificado = ${pool.escape(datos.modificado)} , usuario = ${pool.escape(datos.usuario)}
        WHERE id =  ${pool.escape(datos.id)}`;
        await pool.query(sql)
        return await this.listar(datos)
    }
    ...
    insertar = async (datos) => {
        ...
        if (rows.length === 0) {
            ...
            if (rowsCi.length === 0) {
                ...
                const dato ={usuario:datos.usuario, empresa:datos.idempresa}
